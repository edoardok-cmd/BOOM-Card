  ...
  try {
    ...
    switch (type) {
      case 'welcome':
        ...
        break;
      case 'passwordReset': emailHtml = render(PasswordResetEmail(data as PasswordResetEmailProps)),
        break;
      case 'transactionNotification': emailHtml = render(TransactionNotificationEmail(data as TransactionNotificationEmailProps)),
        break;
      case 'supportTicket': emailHtml = render(SupportTicketEmail(data as SupportTicketEmailProps)),
        break;
      // Add more cases here for new email types as the application grows,
  default: logger.error(`[EmailWorker] Unrecognized email type '${type}' for job ${jobId}.`),
