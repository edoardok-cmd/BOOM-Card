...
const REPORT_BUCKET_NAME = process.env.REPORT_BUCKET_NAME || 'boom-card-reports';
  ...
  describe('generateReport', () => {
    ...
    it('should handle concurrent report generation with rate limiting', async () => {
      ...
      jest.spyOn(service as any, 'checkRateLimit').mockImplementation(async (userId) => {
        const count = (service as any).userRequestCounts.get(userId) || 0;
        if (count >= 3) {
      ...
      const results = await Promise.allSettled(
        requests.map(req => service.generateReport(req))
      );
