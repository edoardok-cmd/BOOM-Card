...
LocationSchema.pre('save', async function (next) {
    ...
    if (this.coordinates && this.coordinates.coordinates && this.coordinates.coordinates.length === 2) {
        ...
        // Example: If an external geocoding service was integrated:
        // const geocoded = await GeocodingService.geocode(this.address);
        // if (geocoded) {
...
LocationSchema.methods.getDistanceTo = function (targetLat: number, targetLon: number): number {
    ...
    const dLat = (targetLat - thisLat) * Math.PI / 180;
    const dLon = (targetLon - thisLon) * Math.PI / 180;
    const a =
